      ...
      try {
        ...
        if (token) {
          ...
          eventSource.onerror = (err) => {
            console.warn('Mất kết nối với SSE Stream, đang thử kết nối lại sau 3 giây...', err);
            if (eventSource) {
              eventSource.close();
            }
  ...
  const handleMarkAsRead = async (id: string, relatedId?: string) => {
    ...
    const navigateToBooking = () => {
      ...
      if (role === 'admin') {
        ...
        if (titleLower.includes('bảo dưỡng') || titleLower.includes('sẵn sàng thấp') || titleLower.includes('lượng xe')) {
          navigate('/admin/inventory');
        } else if (titleLower.includes('chủ xe') || titleLower.includes('đối tác') || msgLower.includes('chủ xe') || msgLower.includes('đối tác')) {
          navigate('/admin/bookings?tab=ownerRequests');
        } else if (titleLower.includes('chờ duyệt') || msgLower.includes('chờ duyệt')) {
        ...
        const titleLower = noti?.title?.toLowerCase() || '';
        const msgLower = noti?.message?.toLowerCase() || '';
        if (titleLower.includes('hủy') || msgLower.includes('hủy')) {
          navigate('/bookings?status=Cancelled');
        } else if (titleLower.includes('xác nhận') || msgLower.includes('xác nhận')) {
          navigate('/bookings?status=Confirmed');
        } else if (titleLower.includes('thuê') || msgLower.includes('thuê')) {
          navigate('/bookings?status=Ongoing');
        } else {
          navigate('/bookings');
        }
      }
                    ...
                    >
                      {languagesList.map((lang) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-4 py-2 text-xs transition-all hover:bg-white/5 border-none bg-transparent cursor-pointer ${language === lang.code ? 'text-neon font-bold' : 'text-gray-300'
                            }`}
                        >
              ...
              <>
                        ...
                        >
                          ...
                          <div className="flex gap-2 px-4 pb-3 border-b border-white/5">
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer border-none ${notiFilter === 'all'
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer border-none ${notiFilter === 'unread'
                                  ? 'bg-neon/15 text-neon shadow-[0_0_8px_rgba(204,255,0,0.1)]'
                                  : 'bg-transparent text-gray-400 hover:text-white hover:bg-white/5'
                                }`}
                            >
                              ...
                              notifications
                                ...
                                .map((noti) => (
                                  <div
                                    ...
                                    className={`group p-3 flex gap-3 text-left transition-all duration-200 cursor-pointer rounded-lg mx-2 ${noti.isRead
                                        ? 'hover:bg-white/5'
                                  ...
                                  >
                                    ...
                                    <div className="relative w-11 h-11 rounded-full bg-black/40 border border-white/5 flex items-center justify-center shrink-0">
                                      ...
                                      <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center border-2 border-surface shadow-md ${noti.type === 'BookingConfirmed' ? 'bg-neon text-dark' :
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <p className="text-xs text-gray-400 mt-0.5 break-words leading-relaxed">{translateNotificationMessage(noti.message, t)}</p>
                ...
                >
                  ...
                  >
                    ...
                    <span className={`text-[9px] uppercase px-1.5 py-0.5 rounded font-bold ${user.role === 'admin' ? 'bg-neon/10 text-neon border border-neon/20' :
                      ...
                      <>
                        ...
                        >
                          ...
                          <div className="px-4 py-2 border-b border-white/5 flex items-center justify-between text-xs text-gray-400 bg-black/10">
                            ...
                            <span className={`text-[10px] uppercase px-2 py-0.5 rounded-full font-bold ${user.role === 'admin' ? 'bg-neon/10 text-neon border border-neon/20' :
                                user.role === 'staff' ? 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20' :
                                  user.role === 'owner' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' :
                                    'bg-green-500/10 text-green-500 border border-green-500/20'
                              }`}>
              ...
              <div className="grid grid-cols-3 gap-2 bg-surface/50 border border-gray-800 rounded-xl p-1 select-none">
                {languagesList.map((lang) => (
                  <button
                    ...
                    className={`py-2 rounded-lg text-xs font-bold transition-all duration-300 cursor-pointer text-center border-none ${language === lang.code
