  ...
  useEffect(() => {
    ...
    bikes.forEach(bike => {
      ...
      if (bikeLng && bikeLat) {
        ...
        if (isOwnBike) {
          actionButtonsHtml = `
            <a href="/motorbike/${bike._id || bike.id}" style="display: block; text-align: center; background-color: #ccff00; color: #000; font-weight: bold; padding: 8px 0; border-radius: 6px; text-decoration: none; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; transition: opacity 0.2s;">Xem chi tiết</a>
          `;
        } else {
          // Nút Chat chuyển hướng sang trang /chat?with={ownerId}&vehicle={vehicleId}
          actionButtonsHtml = `
            <div style="display: flex; gap: 8px;">
              <a href="/motorbike/${bike._id || bike.id}" style="flex: 1; text-align: center; background-color: #ccff00; color: #000; font-weight: bold; padding: 8px 0; border-radius: 6px; text-decoration: none; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; transition: opacity 0.2s;">Thuê ngay</a>
